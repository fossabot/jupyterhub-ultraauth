...
c.OAuthenticator.client_id = environ.get('CLIENT_ID')  # Get from Brendan
c.OAuthenticator.client_secret = environ.get('CLIENT_SECRET') # Get from Brendan
c.GenericOAuthenticator.token_url = environ.get('TOKEN_URL')# oauth2 provider's token url
c.GenericOAuthenticator.userdata_url = environ.get('USERDATA_URL')# oauth2 provider's endpoint with user data
c.GenericOAuthenticator.userdata_method = 'POST' 
c.GenericOAuthenticator.userdata_params = environ.get('userdata_params')# params to send for userdata endpoint
c.GenericOAuthenticator.username_key = environ.get('USERNAME_KEY')# username key from json returned from user data endpoint
